...
function VotingTab({trip,setTrip,user,db,authUserId}) {
  ...
  const writeVote = async (dir, col, itemId) => {
    ...
    if(existing) {
      if(existing.value === val) await supabase.from("votes").delete().eq("id", existing.id);
      else await supabase.from("votes").update({ value: val }).eq("id", existing.id);
    } else {
      ...
      {(trip.destinations||[]).length > 0 && (
        <div className="vote-card">
          ...
          {trip.destinations.map(d=>(
            <VCard key={d.id} icon="📍" name={d.name}
